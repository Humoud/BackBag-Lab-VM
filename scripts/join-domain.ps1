...
param ([String] $ad_ip, [String] $domain)
...
If (($hostname -eq "wef") -or ($hostname -eq "exchange")) {
  ...
  While ($tries -lt 3) {
    Try {
      $tries += 1
      Add-Computer -DomainName $domain -credential $DomainCred -OUPath "ou=Servers,dc="+$domain.split('.')[0]+",dc="+$domain.split('.')[1] -PassThru -ErrorAction Stop
      Break
    } Catch {
      $tries += 1
  ...
  Add-Computer -DomainName $domain -credential $DomainCred -OUPath "ou=Workstations,dc="+$domain.split('.')[0]+",dc="+$domain.split('.')[1]
} Else {
